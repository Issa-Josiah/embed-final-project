...
// ------------------ GOOGLE SHEETS ------------------
#define GOOGLE_SHEETS_URL "https://script.google.com/macros/s/AKfycbz2A0j4zMiMRSiEz3LO1zPNJyeNmQU8PYRarQcafEjSS10ilps96jQnDg3_REPaRp9K/exec"
  ...
  server.on("/logs", []() {
    ...
    if (SPIFFS.exists(LOG_FILE)) {
        ...
        while (f && f.available()) {
            ...
            String datetime = line.substring(0, p1);
            datetime.trim();
            String user = line.substring(p1 + 1, p2);
            user.trim();
            String eventType = line.substring(p2 + 1, p3);
            eventType.trim();
            String tempStr = line.substring(p3 + 1, p4);
            tempStr.trim();
            String weightStr = line.substring(p4 + 1);
            weightStr.trim();
  ...
  for (;;) {
    if (xQueueReceive(logQueue, &item, portMAX_DELAY) == pdTRUE) {
      ...
      if (https.begin(client, GOOGLE_SHEETS_URL)) {
        ...
        "{\"Date\":\"%s\",\"Time\":\"%s\",\"Temperature\":%.1f,"
        "\"Weight\":%.2f,\"User\":\"%s\",\"Event\":\"%s\"}",
        item.date.c_str(), item.time.c_str(),
        item.temp, item.weight,
        item.user.c_str(), item.eventType.c_str()
        );
        ...
        if (httpCode < 200 || httpCode >= 300) {
          ...
          String log = item.date + " " + item.time + " | " + item.user + " | " + item.eventType + " | " +
...
void queueLog(String user, String eventType) {
    ...
    // ALWAYS save for dashboard
    String localLog = date + " " + time + " | " + user + " | " + eventType + " | " +
    ...
    if (xQueueSend(logQueue, &item, pdMS_TO_TICKS(100)) != pdTRUE) {
    saveLog(localLog);
  }
}
...
void setup() {
  ...
  users.push_back({"D7DAE45", "Admin"});
  users.push_back({"202E75D5", "Manager"});
